...
function attachCustomerEventListeners(container, state, onStateChange, menu) {
  ...
  if (googleBtn) {
    googleBtn.addEventListener('click', async () => {
      ...
      googleBtn.innerHTML = `<span>⏳</span><span>Google'a Yönlendiriliyor...</span>`;
      try {
        // Loader göster (redirect öncesi)
        if (typeof window.showGlobalLoader === 'function') window.showGlobalLoader();
        await signInWithGoogle();
        // signInWithRedirect sayfayı yönlendirdiği için buraya normalde gelinmez
      } catch (err) {
